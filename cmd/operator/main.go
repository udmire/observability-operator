...
import (
	...
	"github.com/udmire/observability-operator/pkg/operator"
	util_log "github.com/udmire/observability-operator/pkg/utils/log"
	"github.com/udmire/observability-operator/pkg/utils/usage"
	"github.com/udmire/observability-operator/pkg/utils/version"
...
func main() {
	...
	if mainFlags.printVersion {
		fmt.Fprintln(os.Stdout, version.Print("Observperator"))
		return
	}
	...
	util_log.InitLogger(&cfg.Logging)
	...
	t, err := operator.New(cfg, prometheus.DefaultRegisterer)
	util_log.CheckFatal("initializing application", err)

	if mainFlags.printModules {
		allDeps := t.ModuleManager.DependenciesForModule(operator.All)
...
func LoadConfig(filename string, expandEnv bool, cfg *operator.Config) error {
	...
	if err := dec.Decode((*operator.Config)(cfg)); err != nil {
...
func DumpYaml(cfg *operator.Config) {
